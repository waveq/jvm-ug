...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		if(response == 11) {
			...
			TimeForAllocationsTest.execute(5, true);
		} else if(response == 22) {
			AllocationsInTimeTest.execute(5, true);
		} else if(response == 31) {
			TimeForAllocationsTest.execute(1, false);
		} else if(response == 32) {
			AllocationsInTimeTest.execute(1, false);
		} else if(response == 41) {
			TimeForAllocationsTest.execute(5, false);
		} else if(response == 42) {
			AllocationsInTimeTest.execute(5, false);
		}
	}
}
